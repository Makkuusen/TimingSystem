...
public class RaceAnnouncements {
    ...
    public static void sendFinish(RaceDriver raceDriver, int position, long time)
    {
        sendAnnouncement(raceDriver.race, "messages.announcements.finish", "%player%", raceDriver.getTSPlayer().getName(), "%position%", String.valueOf(position), "%time%", ApiUtilities.formatAsTime(time));
    }
}
