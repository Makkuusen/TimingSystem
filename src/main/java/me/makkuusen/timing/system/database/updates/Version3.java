...
public class Version3 {
    ...
    public static void updateMySQL() throws SQLException{
        DB.executeUpdate("ALTER TABLE `ts_players` ADD `shortName` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL after `name`;");
        DB.executeUpdate("ALTER TABLE `ts_tracks` ADD `timeTrial` tinyint(1) NOT NULL DEFAULT 1 after `type`;");
        DB.executeUpdate("ALTER TABLE `ts_tracks` DROP COLUMN `mode`;");
        DB.executeUpdate("UPDATE `ts_regions` SET `regionIndex` = 1 WHERE `regionType` = 'LAGSTART' OR `regionType` = 'LAGEND';");
    }
